...
def gather_data(coin_name, interval):
    ...
    data.columns = ['open_time', 'o', 'h', 'l', 'c', 'v', 'close_time', 'qav', 'num_trades', 'taker_base_vol', 'taker_quot_vol', 'ignore']
...
def simulate():
    ...
    while (starting_time < dt.datetime.now()):
        ...
        for i in range(len(granular_data.index)):
            date = dt.datetime.fromtimestamp(granular_data.close_time.iloc[i] / 1000.0)
            ...
            if(date.day == current_day.day and date.month == current_day.month):
                ...
                for j in range(len(granular_data.index)):
                    d = dt.datetime.fromtimestamp(granular_data.close_time.iloc[j] / 1000.0)
                ...
                if action == Action.BUY and state.current_balance > 0:
                    ...
                    # Draw buy line
                    plt.axvline(x = granular_data.index[i], color = 'b', label = 'BUY')

                    # Report the buy action
                    print("BUY action AMOUNT: " + str(amount) + "$ BTC: " + str(state.current_bitcoins) + "B PRICE: " + str(state.last_purchase_price))
                    ...
                    # Draw sell line
                    plt.axvline(x = granular_data.index[i], color = 'g', label = 'SELL')

                    # Report the sell action
                    print("SELL action BALANCE: " + str(state.current_balance) + "$ PRICE: " + str(current_price))
